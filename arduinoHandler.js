...
async function ensureCloudRuntimeDependencies(cloudRuntime, onProgress, options = {}) {
  if (cloudRuntime) {
    await installLibrary("ArduinoJson", "latest", onProgress, { signal: options.signal });
  }

  if (!hasStrictMqttRuntimeConfig(cloudRuntime)) {
    return;
  }

  await installLibrary("PubSubClient", "latest", onProgress, { signal: options.signal });
}
